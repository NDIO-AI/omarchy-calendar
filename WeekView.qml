...
Item {
    ...
    function minuteAt(value) {
        var minute = startHour * 60 + Math.round(((Number(value) - topPadding) * 60 / hourHeight) / 15) * 15;
    ...
    function draftY() {
        ...
        return topPadding + (Number(parts[0]) * 60 + Number(parts[1]) - startHour * 60) * hourHeight / 60;
    }
    function draftHeight() {
        ...
        return Math.max(Style.space(24), ((Number(end[0]) * 60 + Number(end[1])) - (Number(start[0]) * 60 + Number(start[1]))) * hourHeight / 60);
    }
                    ...
                    Column {
                        ...
                        Repeater {
                            ...
                            Rectangle {
                                ...
                                DragHandler {
                                    target: null
                                    onActiveChanged: if (!active && activeTranslation.x !== 0) {
                                        root.eventDragged(String(modelData.uid || ""), Math.round(activeTranslation.x / allDayCell.width), 0);
                                    }
                                }
                            }
                        }
        ...
        Flickable {
                    ...
                    Rectangle {
                        ...
                        TapHandler {
                            ...
                            onTapped: function (eventPoint) {
                                root.emptySlotRequested(timedColumn.day, root.minuteAt(eventPoint.position.y));
                            }
                        }
                        ...
                        Repeater {
                            ...
                            Rectangle {
                                ...
                                DragHandler {
                                    id: moveHandler
                                    target: null
                                    onActiveChanged: if (!active && (activeTranslation.x !== 0 || activeTranslation.y !== 0)) {
                                        root.eventDragged(String(modelData.uid || ""), Math.round(activeTranslation.x / timedColumn.width), Math.round((activeTranslation.y * 60 / root.hourHeight) / 15) * 15);
                                    }
                                }
                                Rectangle {
                                    ...
                                    DragHandler {
                                        target: null
                                        xAxis.enabled: false
                                        onActiveChanged: if (!active && activeTranslation.y !== 0)
                                            root.eventResized(String(modelData.uid || ""), Math.round((activeTranslation.y * 60 / root.hourHeight) / 15) * 15)
                                    }
                                }
                            }
                        }
                        ...
                        Rectangle {
                            ...
                            visible: root.isCurrentWeek() && CalendarModel.dayKey(timedColumn.day) === CalendarModel.dayKey(root.nowTime) && root.currentTimeY() >= root.topPadding && root.currentTimeY() <= root.gridHeight + root.topPadding
                            x: 0
                            y: root.currentTimeY()
                            width: parent.width
                            height: 2
            ...
            Rectangle {
                ...
                DragHandler {
                    ...
                    target: null
                    onActiveChanged: if (!active && (activeTranslation.x !== 0 || activeTranslation.y !== 0)) {
                        root.eventDragged(root.editingUid, Math.round(activeTranslation.x / ((gridFlick.width - root.timeGutter) / 7)), Math.round((activeTranslation.y * 60 / root.hourHeight) / 15) * 15);
                    }
                }
                Text {
                    textFormat: Text.PlainText
                ...
                Rectangle {
                    ...
                    DragHandler {
                        ...
                        xAxis.enabled: false
                        onActiveChanged: if (!active && activeTranslation.y !== 0)
                            root.eventResized(root.editingUid, Math.round((activeTranslation.y * 60 / root.hourHeight) / 15) * 15)
                    }
                }
            }
        }
