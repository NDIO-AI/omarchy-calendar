...
Rectangle {
    ...
    readonly property var metadataRows: [
        {
            label: "Calendar",
            value: eventData ? String(eventData.calendar_name || "Unknown") : "None selected"
        },
        {
            label: "Provider",
            value: eventData ? CalendarModel.providerLabel(eventData.provider) : "None selected"
        },
        {
            label: "Account",
            value: eventData ? String(eventData.account_label || "Local account") : "None selected"
        },
        {
            label: "Sync",
            value: providerStatus || (eventData ? "Local cache" : "Not available")
        }
    ]
    ...
    Column {
        ...
        Text {
            ...
            textFormat: Text.PlainText
            width: parent.width
            text: root.eventData ? CalendarModel.formatTime(root.eventData) : "Use j and k to move through the agenda."
    ...
    Row {
        ...
        Rectangle {
            ...
            MouseArea {
                anchors.fill: parent
                enabled: Boolean(root.eventData)
        ...
        Flickable {
            ...
            Column {
                ...
                Text {
                    ...
                    textFormat: Text.PlainText
                    width: parent.width
                    text: root.eventData && root.eventData.description ? String(root.eventData.description) : root.eventData ? "No description was provided." : "Select an event to see its details."
                    color: root.eventData && root.eventData.description ? root.palette.foreground : root.palette.muted
                    font.family: root.fontFamily
                    font.pixelSize: Style.font.bodySmall * root.textScale
                    lineHeight: 1.35
                    wrapMode: Text.Wrap
            ...
            Rectangle {
                ...
                Rectangle {
                    ...
                    y: detailScroll.contentHeight <= detailScroll.height ? 0 : (parent.height - height) * detailScroll.contentY / (detailScroll.contentHeight - detailScroll.height)
                    radius: width / 2
                    color: root.palette.accent
                }
            }
